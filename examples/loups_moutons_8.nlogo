...
to init_moutons
  create-moutons nombre_de_moutons [ init_mouton ]
end
...
to init_loups
  create-loups nombre_de_loups [ init_loup ]
end
...
to suivre_un_mouton
  let nourriture min-one-of moutons [ distance myself ] ; trouver le mouton le plus proche
  if (nourriture != nobody)                             ; si il y a de la nourriture à proximité
  [ set heading towards nourriture ]
end

to avancer_loups
  ifelse (energie < 75)                   ; si le loup a faim
  [ set color orange
    suivre_un_mouton
  ]
  [ set color yellow
...
650
625
Population de loups et de moutons
NIL
NIL
0.0
10.0
